...
describe('build-project-files', () => {
  ...
  describe('generate loader', () => {
    ...
    it('includes the generted banner', () => {
      const preamble = generatePreamble(config); // lack of DI makes this harder to test, so will just pre-calc
      expect.assertions(1);
      return callGenerateLoader().then(res => expect(res).toEqual(preamble));
    });

    it('includes the injected app', () => {
      expect.assertions(1);
      mockGetClientCoreFile.mockReturnValue(Promise.resolve(`pretend i am code ('__STENCIL__APP__') yeah me too`));
      return callGenerateLoader().then(res => {
        let lines = res.split('\n');
        expect(lines[1]).toEqual(`pretend i am code ("MyApp","build/myapp/myapp.core.js","build/myapp/myapp.core.pf.js",[]) yeah me too`);
      });
    });
  });
